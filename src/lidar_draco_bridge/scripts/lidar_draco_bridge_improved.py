...
class ImprovedLidarDracoBridge(Node):
    """
    개선된 ROS2 노드: point_cloud_transport_py를 사용한 효율적인 라이다-드라코 브리지
    """
    
    def __init__(self):
        super().__init__('improved_lidar_draco_bridge')
        ...
        self.get_logger().info(f"Improved Lidar-Draco Bridge started")
    ...
    def print_statistics(self):
        ...
        if self.message_count > 0:
            ...
            self.get_logger().info("=== Improved Bridge Statistics ===")
...
def main(args=None):
    ...
    try:
        bridge = ImprovedLidarDracoBridge()
        rclpy.spin(bridge)
